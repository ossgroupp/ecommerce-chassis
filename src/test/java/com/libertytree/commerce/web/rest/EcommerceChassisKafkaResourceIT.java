...
import com.libertytree.commerce.config.KafkaSseConsumer;
import com.libertytree.commerce.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class EcommerceChassisKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/ecommerce-chassis-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/ecommerce-chassis-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/ecommerce-chassis-kafka/unregister"));
                return;
            }
        }
